<template>
  ...
  <a-popconfirm v-model:popup-visible="popupVisible" @ok="onOk" v-if="!isLoading && !isRun" content="跳过此次休息,重新开始专注倒计时">
    <div v-show="!isLoading" id="setTimeModal" style="display: flex; align-items: center;"
      class="py-1 px-2 rounded-md pointer-events-none">
      <input id="minute" type="text" class="text-center pointer-events-none"
        :value="formatTime(timerService.minutes)" />
      <span style="padding: 0 5px">:</span>
      <input id="second" type="text" class="text-center pointer-events-none"
        :value="formatTime(timerService.seconds)" />
      <a-tooltip v-if="!isRun" position="br" @click.stop="void 0">
        <div class="flex ml-2 items-center justify-center pointer-events-auto" style="height: 25px">
          <icon-info-circle size="16px" class="cursor-pointer " />
        </div>
        <template #content>
          <div class="text-sm font-medium shadow-md">
...
export default {
  ...
  setup(props) {
      ...
      if (timerService.isRunning) {
        ...
        timerService.timer = setInterval(() => {
          if (timerService.seconds === TIP_TIME && timerService.minutes === 0) {
            ...
            window.utools.showNotification(message + '点击可取消', "live2d-confirm");
    ...
    const resetTimer = (params) => {
      if (isEmpty(params)) {
        const settings = getStorageItem("settings");
        if (settings) {
          params = { minutes: settings.focusTime, seconds: 0 };
        } else {
          params = { minutes: 25, seconds: 0 };
        }
      }
      clearInterval(timerService.timer);
      timerService.minutes = getMinutes(params.minutes || 25);
      timerService.seconds = params.seconds || 0;
      timerService.isRunning = false;
      toggleTimer();
    };
    ...
    const onOk = () => {
      window.service.onResetBeakTime()
    ...
    onMounted(() => {
      ...
      emitter.off('open-confirm');
      emitter.on('open-confirm', openConfirm);
