<template>
    <a-button shape="circle" @click="handleClick" style="--color-secondary: #fff">
        <icon-settings class="!w-5 !h-5" />
    </a-button>

    <a-modal v-model:visible="visible" title-align="start" modal-class="right-4 !absolute" :modal-style="{
        boxShadow: '0 4px 10px rgba(0, 0, 0, 0.1)'
    }" unmount-on-close :esc-to-close="false" :mask="false" :align-center="false" :top="60" @ok="handleOk"
        @cancel="handleCancel">
        <template #title>
            偏好设置
        </template>
            ...
            <div>
                <p class="flex items-center text-sm text-gray-600 ">
                    <svg t="1755921599827" class="mr-1" viewBox="0 0 1024 1024" version="1.1"
                        xmlns="http://www.w3.org/2000/svg" p-id="6035" width="18" height="18">
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="6036"></path>
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="6037"></path>
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="6038"></path>
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="6039"></path>
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="6040"></path>
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="6041"></path>
                    </svg>
                    <span>专注时间</span>
                </p>
                <p class="text-center text-lg font-semibold">{{ focusTime }}分钟</p>
                <a-slider class="slider" :show-tooltip="false" v-model="focusTime" :min="1" :max="60" :step="1" :marks="{
                    1: '1分钟',
                    60: '60分钟'
                }" />
            </div>
                ...
                <p class="flex items-center text-sm text-gray-600">
                    <svg t="1755921722533" class="mr-1" viewBox="0 0 1024 1024" version="1.1"
                        ...
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="7371"></path>
                        <path
                            ...
                            fill="rgb(75, 85, 99)" p-id="7372"></path>
                    </svg>
                    <span>休息时间</span>
                </p>
                <p class="text-center text-lg font-semibold">{{ breakTime }}分钟</p>
                <a-slider class="slider" :show-tooltip="false" v-model="breakTime" :min="1" :max="60" :step="1" :marks="{
                    1: '1分钟',
                    60: '60分钟'
                }" />
            </div>
...
import { shallowRef, watchEffect, getCurrentInstance } from "vue";
...
export default {
    ...
    setup() {
        const focusTime = shallowRef(25);
        const breakTime = shallowRef(5);
        ...
        const handleOk = () => {
            ...
            setStorageItem('settings', time)
            emitter.emit('reset-timer', { minutes: time.focusTime, seconds: 0 })
            handleCancel()
        }
        ...
        watchEffect(() => {
            if (visible.value) {
                ...
                if (settings) {
                    focusTime.value = settings.focusTime || 25;
...
<style lang="css">
.slider .arco-slider-mark {
    white-space: nowrap;
}

.slider .arco-slider-track::before {
    height: 4px;
}

.slider .arco-slider-bar {
    height: 4px;
}

.slider .arco-slider-dot {
